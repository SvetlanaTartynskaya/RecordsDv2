...
def show_next_equipment(update: Update, context: CallbackContext, user_type='user'):
    ...
    try:
        ...
        validator = MeterValidator()
        inv_num = equipment[current_idx]['Инв. №']
        meter_type = equipment[current_idx]['Счётчик']
        last_reading = validator._get_last_reading(inv_num, meter_type)
        
        # 3. Форматирование сообщения
        message = (
            f"Оборудование {current_idx+1}/{len(equipment)}\n"
            f"Гос.номер: {equipment[current_idx]['Гос. номер']}\n"
            f"Инв.№: {inv_num}\n"
            f"Счётчик: {meter_type}\n"
            f"{'Последнее: ' + str(last_reading['reading']) if last_reading else ''}"
        )
        ...
        # 5. Отправка с учетом типа update
        if update.callback_query:
            update.callback_query.edit_message_text(message, reply_markup=InlineKeyboardMarkup(buttons))
        else:
